...
class Migration0006FreshInstallTestCase(TransactionTestCase):
    ...
    def test_migration_function_handles_missing_table(self):
        ...
        mock_schema_editor.connection.introspection.get_table_description.side_effect = (
            DatabaseError("Table doesn't exist")
        )

        # Mock the model import to return the actual model
        with patch.object(migration_module, "RecurringUserPlan", RecurringUserPlan):
...
class Migration0006ReverseTestCase(TransactionTestCase):
    ...
    def test_reverse_migration_removes_wallet_fields(self):
        ...
        column_names_before = [col.name for col in table_description]
        self.assertIn("status", column_names_before, "status should exist before reverse")
        self.assertIn(
        ...
        self.assertNotIn(
            "extra_data", column_names_after, "extra_data should be removed after reverse"
        )

        # Verify original fields still exist
        self.assertIn("token", column_names_after, "token should still exist")
        self.assertIn(
            "payment_provider", column_names_after, "payment_provider should still exist"
        )
        self.assertIn("user_plan_id", column_names_after, "user_plan_id should still exist")
    ...
    def test_reverse_migration_preserves_original_data(self):
        ...
        self.assertEqual(row[1], original_provider, "payment_provider should be preserved")
        self.assertEqual(row[2], original_verified, "token_verified should be preserved")
    ...
    def test_reverse_and_forward_migration_cycle(self):
        ...
        # Store original data
        original_token = self.recurring.token
        original_status = self.recurring.status
        original_extra_data = self.recurring.extra_data
        ...
        self.assertIn("extra_data", column_names, "extra_data should exist after re-apply")
