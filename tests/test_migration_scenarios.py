...
IMPORTANT: These tests currently FAIL due to a migration bug in 0007_recurringuserplan_basewallet_fields.py.
...
@skipUnless(
    getattr(settings, "PLANS_RECURRINGUSERPLAN_MODEL", None)
    == "plans.RecurringUserPlan",
    "requires DJANGO_SETTINGS_MODULE=tests.settings_scenario1 (unswapped start)",
)
class Scenario1UpgradeFromPlansTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        call_command(
            "migrate",
            "plans_payments",
            "0007_recurringuserplan_basewallet_fields",
            verbosity=0,
            interactive=False,
        )
...
@skipUnless(
    str(getattr(settings, "PLANS_RECURRINGUSERPLAN_MODEL", "")).startswith(
        "plans_payments."
    ),
    "requires the plans_payments.RecurringUserPlan swap",
)
